...
if HAS_MCP:
    ...
    @mcp.tool(description="Crea un informe completo con múltiples hojas, tablas y gráficos en un solo paso. Ideal para generar reportes profesionales y personalizados con estructura compleja.")
    def create_report_tool(file_path, data, report_config, overwrite=False):
        """
        Crea un informe completo con múltiples hojas, tablas y gráficos en un solo paso.
        ...
        Args:
            ...
            report_config (dict): Configuración del informe:
                {
                    "tables": [
                        {
                            "sheet": "sheet_name",
        ...
        try:
            ...
            # Crear tablas
            for table_config in report_config.get("tables", []):
                sheet_name = table_config["sheet"]
                table_name = table_config["name"]
                range_str = table_config["range"]
                style = table_config.get("style", "TableStyleMedium9")
            ...
            # Crear gráficos
            for chart_config in report_config.get("charts", []):
                sheet_name = chart_config["sheet"]
                chart_type = chart_config["type"]
                data_range = chart_config["data_range"]
                title = chart_config.get("title", f"Chart {len(wb[sheet_name]._charts) + 1}")
                ...
                # Crear gráfico
                add_chart(wb, sheet_name, chart_type, data_range, title, position, style)
            
            # Aplicar formatos
            for format_config in report_config.get("formats", []):
                sheet_name = format_config["sheet"]
                range_str = format_config["range"]
                
                if sheet_name not in wb.sheetnames:
                    logger.warning(f"La hoja '{sheet_name}' no existe para aplicar formato")
                    continue
                
                ws = wb[sheet_name]
                
                if "number_format" in format_config:
                    apply_number_format(ws, range_str, format_config["number_format"])
                
                if "style" in format_config:
                    apply_style(ws, range_str, format_config["style"])
            ...
            return {
                "success": True,
                "file_path": file_path,
                "sheets_created": list(data.keys()),
                "tables_created": len(report_config.get("tables", [])),
                "charts_created": len(report_config.get("charts", [])),
                "message": f"Informe creado correctamente: {file_path}"
            }
        except Exception as e:
            logger.error(f"Error al crear informe: {e}")
            return {
                "success": False,
                "error": str(e),
                "message": f"Error al crear informe: {e}"
            }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "create_report" and len(sys.argv) > 2:
            ...
            data = {
                "Ventas": [
                    ...
                    ["Producto A", 100, 150, 130],
                    ["Producto B", 200, 180, 210],
                    ["Producto C", 120, 140, 160]
                ]
            }
            ...
            result = create_report_tool(file_path, data, config, True)
            ...
            logger.info("Uso: python master_excel_mcp.py create_report archivo.xlsx")
